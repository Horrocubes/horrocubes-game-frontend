...
const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    let amount = EmurgoSerialization.Value.new(EmurgoSerialization.BigNum.from_str("0"));
    ...
    for (let i = 0; i < splitOutputsAmounts.length; i++) {
      ...
      let range = {};
      range.ideal = EmurgoSerialization.Value.new(
        EmurgoSerialization.BigNum.from_str("0")
      )
        .checked_add(splitOutputsAmounts[i])
        .checked_add(splitOutputsAmounts[i]);
      range.maximum = EmurgoSerialization.Value.new(
        EmurgoSerialization.BigNum.from_str("0")
      )
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      maxFee = EmurgoSerialization.Value.new(
        EmurgoSerialization.BigNum.from_str(maxFee.toString())
      );
      ...
      if (compare(change, minAmount) < 0) {
        ...
        const minAda = minAmount
          .checked_sub(EmurgoSerialization.Value.new(change.coin()))
          .checked_add(EmurgoSerialization.Value.new(utxoSelection.amount.coin()));
...
function improve(utxoSelection, outputAmount, limit, range) {
  ...
  const newAmount = EmurgoSerialization.Value.new(
    EmurgoSerialization.BigNum.from_str("0")
  )
...
function mergeOutputsAmounts(outputs) {
  let compiledAmountList = EmurgoSerialization.Value.new(
    EmurgoSerialization.BigNum.from_str("0")
  );
...
function splitAmounts(amounts) {
  ...
  if (amounts.multiasset()) {
    ...
    for (let i = 0; i < mA.keys().len(); i++) {
      ...
      for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
        let _assets = EmurgoSerialization.Assets.new();
        let assetName = mA.get(scriptHash).keys().get(j);

        _assets.insert(
          EmurgoSerialization.AssetName.from_bytes(assetName.to_bytes()),
          EmurgoSerialization.BigNum.from_bytes(
            mA.get(scriptHash).get(assetName).to_bytes()
          )
        );

        let _multiasset = EmurgoSerialization.MultiAsset.new();
        _multiasset.insert(
          EmurgoSerialization.ScriptHash.from_bytes(scriptHash.to_bytes()),
          _assets
        );
        let _value = EmurgoSerialization.Value.new(
          EmurgoSerialization.BigNum.from_str("0")
        );
  ...
  splitAmounts.push(
    EmurgoSerialization.Value.new(
      EmurgoSerialization.BigNum.from_bytes(amounts.coin().to_bytes())
    )
  );
...
const cloneUTxOList = (utxoList) =>
  utxoList.map((utxo) =>
    EmurgoSerialization.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
  );
 ...
 */
const cloneValue = (value) => EmurgoSerialization.Value.from_bytes(value.to_bytes());
