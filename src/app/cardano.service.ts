...
/*
 ...
 valueToAssets(value: Value)
 {
  ...
  getRedeemer(level: string, answer) {
  ...
  initTx()
  {
    let protocolParameters = this.getProtocolParameters();
    const txBuilder = EmurgoSerialization.TransactionBuilder.new(
      ...
      EmurgoSerialization.LanguageViews.new(Buffer.from(this.getLanguageViews(), "hex"))
    );  
    const datums = EmurgoSerialization.PlutusList.new();
    const metadata = { [this.DATUM_LABEL]: {}, [this.ADDRESS_LABEL]: {} };
   ...
   */
   async finalizeTx(
    ...
    if (scriptUtxo) {
      ...
      const collateral = (await this._cardanoRef.cardano.getCollateral()).map((utxo) =>
      ...
      Object.keys(metadata).forEach((label) => {
        Object.keys(metadata[label]).length > 0 &&
          generalMetadata.insert(
            EmurgoSerialization.BigNum.from_str(label),
            EmurgoSerialization.encode_json_str_to_metadatum(
    ...
    if (
      changeMultiAssets &&
      change.to_bytes().length * 2 > this.getProtocolParameters().maxValSize
    ) {
        ...
        for (let j = 0; j < changeMultiAssets.len(); j++) {
          ...
          for (let k = 0; k < assetNames.len(); k++) {
            ...
            if (
              ...
              this.getProtocolParameters().maxValSize
            ) {
      ...
      const minAda = EmurgoSerialization.min_ada_required(
        partialChange,
        EmurgoSerialization.BigNum.from_str(this.getProtocolParameters().minUtxo)
      );
...
getSerializationLibDatumHash(level)
{
  const levels = new Map([
...
getSerializationLibDatumData(level)
{
  const levels = new Map([
    [0, "d866820081c24100"],
    [1, "d86682008101"],
